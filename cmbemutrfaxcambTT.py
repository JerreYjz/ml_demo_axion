...
#print(device)
extrainfo = np.load("extraaxcambTT.npy",allow_pickle=True)
...
covinv = np.load('cosvarinvTT.npy',allow_pickle=True)[:camb_ell_range,:camb_ell_range]*4/np.exp(4*0.06)
...
train_data_vectors = np.load('trainaxcamb_output.npy',allow_pickle=True)[:,:,0]

validation_data_vectors = np.load('valiaxcamb_output.npy',allow_pickle=True)[:,:,0]
...
PATH = "./chiTTAstautrfaxcamb"+str(nc)#rename as drop0 afterward
