...
from cobaya.theories.emulcmbtrfax.emulator import Supact, Affine, Better_Attention, Better_Transformer, ResBlock, TRF
import joblib
import scipy
from sklearn.gaussian_process import GaussianProcessRegressor
from sklearn.gaussian_process.kernels import RBF

class emulcmbtrfax(BoltzmannBase):
    aliases: dict = {
        "omega_b" : [ "omegabh2" ],
        "omega_cdm" : [ "omegach2" ],
        "H_0" : [ "H0" ],
    ...
    def initialize(self):
        ...
        self.model1 = TRF(input_dim=8,output_dim=2598,int_dim=intdim, int_trf=inttrf,N_channels=nc)
        self.model2 = TRF(input_dim=8,output_dim=2598,int_dim=intdim, int_trf=inttrf,N_channels=nc)
        self.model3 = TRF(input_dim=8,output_dim=2598,int_dim=intdim, int_trf=inttrf,N_channels=nc)
    ...
    def calculate(self, state, want_derived=True, **params):
        ...
        state["tt"][2:2600] = self.scaletrans(self.predict(self.model1,
                                                           cmb_params,
                                                           self.extrainfo_TT), 
                                              cmb_params)[0]
        state["te"][2:2600] = self.scaletrans(self.predict(self.model2, 
                                                           cmb_params, 
                                                           self.extrainfo_TE), 
                                              cmb_params)[0]
        state["ee"][2:2600] = self.scaletrans(self.predict(self.model3, 
                                                           cmb_params, 
                                                           self.extrainfo_EE), 
                                              cmb_params)[0]
        state["et"] = state["te"]
